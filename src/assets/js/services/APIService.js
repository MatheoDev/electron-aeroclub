class APISerice {
    ...
    getAvions(write) {
        fetch(this.url+'/avions', this.config)
            ...
            .then((json) => {
                if (write) {
                    let html = '<h3>Les avions</h3><a class="btn-new btn btn-info" href="_newAvion.html">Ajouter un avion</a><table class="table"><thead class="thead-dark"><tr><th scope="col">num</th><th scope="col">type</th><th scope="col">immat</th><th scope="col">action</th></tr></thead><tbody>'
    ...
    getForfaits(write) {
        fetch(this.url+'/forfaits', this.config)
            ...
            .then((json) => {
                if (write) {
                    let html = '<h3>Les forfaits</h3><a class="btn-new btn btn-info" href="_newForfait.html">Ajouter un forfait</a><table class="table"><thead class="thead-dark"><tr><th scope="col">id</th><th scope="col">libelle</th><th scope="col">heure</th><th scope="col">prix</th><th scope="col">action</th></tr></thead><tbody>'
                    json.forEach(element => {
